...
void mandelbrot_interrupt_col_loop(double x0, double y0, double x1, double y1,
  ...
  for(; ; ) { // col loop
    int col_lo2 = col_lo;
    int col_hi2 = MIN(col_lo + D_col, col_hi);
    for (; col_lo2 < col_hi2; ++col_lo2) {
      ...
      for (; ; ) {
        int row_lo2 = row_lo;
        int row_hi2 = MIN(row_lo + D_row, row_hi);
        for (; row_lo2 < row_hi2; ++row_lo2) { // row loop
          double z_real = x0 + row_lo2*xstep;
          double z_imaginary = y0 + col_lo2*ystep;
          double c_real = z_real;
          double c_imaginary = z_imaginary;
          double depth = 0;
          while(depth < max_depth) {
          ...
          output[col_lo2*width + row_lo2] = static_cast<unsigned char>(static_cast<double>(depth) / max_depth * 255);
        }
        row_lo = row_lo2;
        ...
        if (unlikely(heartbeat)) {
          if (row_loop_handler(x0, y0, x1, y1, width, height, max_depth, output, xstep, ystep, col_lo2, col_hi, row_lo, row_hi, p)) {
            return;
          }
        }
      }
